...
def getSession(driver):
    ...
    for index in range(0, len(options_term)):
        print (options_term[index].text)
    
    #gets the term the user wants by looping until the user enters Exit or a valid term
    foundTerm = False
    while (foundTerm == False):
        ...
        for k in class_list[i].keys():
            ...
            if (len(class_list[i]) == 2):
                if (k == "name"):
                    ...
                    rcs_id = "error_"+ first_name + "_" + last_name    
...
def getInfoFromCourse(driver):
    ...
    for index in range(0, len(options_course)):
        ...
        while (foundAnswer == False):
            ...
            print ("Do you want pictures from " + course +"?")
