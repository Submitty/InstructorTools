...
def cleanup():
    ...
    pathlib.Path(notes_dir,exist_ok=True)
    pathlib.Path(qr_dir,exist_ok=True)
...
def make_all_exams():
    ...
    with open(seating_file,"r") as seating:
        for line in seating:
            ...
            make_custom_pdf("test_template.pdf","student_notes/"+studentnotes,"overlay.pdf",filename)
